...
#' ### ACRO Summarise
#'
#' - ACRO also supports the familiar summarise() function from the tidyverse package.
#' Below is an example of a standard piping operation that uses acro_summarise() instead of tidyverse summarise().
#' - In this example, the data is grouped by *parents* and *recommendation*, and we calculate *the mean number of children*.
#'
## ---------------------------------------------------------------------------------------------------------------------
...
#'
#' As you can see from the outputs, acro_summarise() produces:
#' - *The summary table*: Identical results to what standard tidyverse summarise() would generate.
#' - *Disclosure metrics (counts and dominance)*: These are the Statistical Disclosure Control (SDC) checks that ACRO performs automatically in the background. They provide the evidence required by a Trusted Research Environment (TRE) output checker, saving you from having to calculate these checks manually.
#'
#' ### ACRO Crosstab
#'
...
#'
#' -   **Tables** via `acro_table()`, `acro_crosstab()`, `acro_pivot_table()` and `acro_summarise()`.
#'       For the `acro_table()`, `acro_crosstab()`, `acro_pivot_table()`:
...
#'
#'       For acro_summarise():
#'     -   supported aggregation functions are: *mean*, *median*, *sum*, *std*, and
#'         *mode*. The support for *n()* will be available shortly.
